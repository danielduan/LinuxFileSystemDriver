...
ospfs_dir_readdir(struct file *filp, void *dirent, filldir_t filldir)
{
	...
	while (r == 0 && ok_so_far >= 0 && f_pos >= 2) {
		...
		//break;		/* Fix me! */
		 ...
		 curr_file_offset = (f_pos - 2) * OSPFS_DIRENTRY_SIZE;
		 if(curr_file_offset >= dir_oi->oi_size){
		 	 r = 1;
		 	 break;
		 }
		...
		/* EXERCISE: Your code here */
		 ...
		 //Read directory entry at the offset
		 od = ospfs_inode_data(dir_oi,curr_file_offset);

		 //Get inode
		 entry_oi = ospfs_inode(od->od_ino);

		 // We ignore the blank directory entry
...
add_block(ospfs_inode_t *oi)
{
	...
	//setup variables
	int32_t index_indirect;
	int32_t index_dindirect;
	int32_t index_direct;
	
	uint32_t *indir_data = NULL;
	uint32_t *dindir_data = NULL;

	uint32_t indir_block = 0;
	uint32_t dindir_block = 0;
	
	const int DINDIR_2_BLOCK = 0;
	...
	//if file size is zero, check if there is allocated block
	//add the next block
	if(oi->oi_direct[0] != 0 && oi->oi_size == 0) {
		n = 1;
	}
	
	index_dindirect = indir2_index(n);
	index_indirect = indir_index(n);
	index_direct = direct_index(n);
	
	//if new block create in inode
	if(index_indirect == -1) {
		//check if there is existing allocated blocks
	...
	//if double indirect is needed
	if(index_dindirect == 0) {
		//if not allocated yet
		if(oi->oi_indirect2 == 0) {
			if((allocated[DINDIR_2_BLOCK] = allocate_block()) == 0) {
				//handle no space left
				if(allocated[INDIR_BLOCK] != 0) {
                                	free_block(allocated[INDIR_BLOCK]);
                        	}
			...
			dindir_block = allocated[DINDIR_2_BLOCK];
			dindir_data = ospfs_block(dindir_block);
			
			memset(dindir_data, 0, OSPFS_BLKSIZE);
		} else {
			dindir_block = oi->oi_indirect2;
			dindir_data = ospfs_block(dindir_block);	
		}
		//allocate indir block
		if (dindir_data[index_indirect] == 0) {
	...
	//check for existing allocated blocks
	...
	//now we can put the variables in their place
	oi->oi_size = (n + 1) * OSPFS_BLKSIZE;
	if(index_dindirect == 0) {
		if(oi->oi_indirect2 == 0) {
			oi->oi_indirect2 = dindir_block;
		}
		if(dindir_data[index_indirect] == 0) {
			dindir_data[index_indirect] = indir_block;
		}
	} else if (oi->oi_indirect == 0) {
...
ospfs_read(struct file *filp, char __user *buffer, size_t count, loff_t *f_pos)
{
	...
	//Prevent overflow as covered in discussion
...
ospfs_symlink(struct inode *dir, struct dentry *dentry, const char *symname)
{
	...
	/* EXERCISE: Your code here. */
	//Initialize our symlink inode
	ospfs_symlink_inode_t *symlink_ino = NULL;
	ospfs_direntry_t *new_entry;

	//Error checking code for TOOLONG and EEXIST
    if(dentry->d_name.len > OSPFS_MAXNAMELEN)
        return -ENAMETOOLONG;
    ...
    //Find free inode and set symlink_ino
    while(entry_ino < ospfs_super->os_ninodes){
	  symlink_ino = ospfs_inode(entry_ino);
	  if(symlink_ino == NULL)
	    return -EIO;
	  if(symlink_ino->oi_nlink == 0)
	    break;
	  entry_ino++;
	}

	//Else couldn't find an inode
	if(entry_ino == ospfs_super->os_ninodes)
		return -ENOSPC;
